...
def ingest(req: IngestRequest, Authorization: Optional[str] = Header(default=None)):
    ...
    timings = ingest_messages(messages, passphrase=req.passphrase, folder_id=req.folder_id, log_timings=True)
...
def query(req: QueryRequest):
    results, t_retr = search_emails(req.question, passphrase=req.passphrase, n_results=req.n_results, log_timings=True)
    if not results:
